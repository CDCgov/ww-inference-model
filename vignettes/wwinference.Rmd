...
Note that by default the `get_draws()` function will return a list of class `wwinference_fit_draws` all of the posterior draws for predicted hospitalizations, wastewater concentration, global, and site $\mathcal{R}(t)$ estimates. To examine a particular variable (e.g. `"predicted counts"` for posterior
