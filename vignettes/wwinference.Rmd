...
data from wastewater treatment plants whose catchment areas
are contained withinthe larger "global" population. For this quick start,
we will usesimulated data, modeled after a hypothetical US state with 4
wastewater treatment plants (also referred to as sites) reporting data on log scale
viral concentrations of SARS-COV-2, processed in 3 different labs, covering about
25% of the state's population. This simulated data contains daily counts of the
...
followign columns. The wastewater data should not contain NAs for days with
missing observations, instead these should be excluded. Additionally,
there should be not be multiple observations within a site, lab, and sample
collection day:
...
- a concentration (column `log_genome_copies_ml`): the natural log of the
measured genome copies per mL from the sample collected. This column should not
contain NAs, even if the observation for that sample is below the limit of
detection. We will
...
```
Note that the `get_draws_df()` function will return a tidy long
dataframe with all of the posterior draws for the calibration, nowcast, and
forecast period joined to applicable data for each of
the included variables in the `name` column:
  - counts(`"predicted counts"`)
  - site-level wastewater concentrations (`"predicted wastewater"`)
  - "global"(e.g. state) R(t) estimate (`"global R(t)"`)
  - "local" (site + the one auxiliary subpopulation) R(t) estimates (`"subpopulation R(t)"`)

If only interested in the hospital admissions estimates
...
include the data that was below the LOD (even though the fit incorporated
them via censoring)
