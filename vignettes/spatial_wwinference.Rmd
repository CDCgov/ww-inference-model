...
function in `corr_fun_params`.  Currently we have four built in functions for
correlation types : `independence_corr_func_r()`,
`exponential_decay_corr_func_r()`, `custom_corr_matrix()`, and
`rand_corr_matrix_func()`.  The default used is
`exponential_decay_corr_func_r()` correlation function based of the following
fake locations.
...
fake_locs_plot <- ggplot(
  ...
  coord_cartesian(
    xlim = c(min(fake_locs$x) - 5, max(fake_locs$x) + 5),
    ylim = c(min(fake_locs$y) - 5, max(fake_locs$y) + 5)
  ) +
  labs(
...
exponential decay correlation function based off distance matrix, and a preset
...
`exponential_decay_corr_func_r()`, `custom_corr_matrix_func()`, and
`rand_corr_matrix_func()`.  Each correlation matrix function, built in or user
created, expects a set of parameters `corr_fun_params` that are specific to that
function.  See each built in function's documentation for details on how to set
these parameters.  Under the hood, the correlation matrix is created by calling
...
sigma_sqrd_generalized <- 0.0095^n_sites
mean_report_freq <- 3 / 7
custom_corr_matrix <- matrix(
  c(
    1, 0.10, 0.25, 0.15,
    0.1, 1, 0.2, 0.96,
    0.25, 0.2, 1, 0.25,
...
simulated_data_iid <- withr::with_seed(2024, {
  wwinference::generate_simulated_data(
    ...
    pop_size = pop_size,
    mean_reporting_freq = mean_report_freq,
...
# ------------------------------------------------------------------------------
# Pre-set Matrix Simulated Data ------------------------------------------------
simulated_data_set_corr_mat <- withr::with_seed(1, {
  wwinference::generate_simulated_data(
    r_in_weeks = r_in_weeks, # nolint
    n_sites = n_sites,
    site = site,
    ...
    global_rt_sd = global_rt_sd,
    use_spatial_corr = TRUE,
    corr_function = custom_corr_matrix_func,
    corr_fun_params = custom_corr_params,
    sigma_sqrd_generalized = sigma_sqrd_generalized,
    aux_site_bool = TRUE
  )
})
...
# Pre-set corr mat case
hosp_data_proc_set_corr_mat <- wwinference::preprocess_count_data(
  simulated_data_set_corr_mat$hosp_data,
  count_col_name = "daily_hosp_admits",
  pop_size_col_name = "state_pop"
)
ww_data_proc_set_corr_mat <- wwinference::preprocess_ww_data(
  simulated_data_set_corr_mat$ww_data
)
ww_data_to_fit_set_corr_mat <- wwinference::indicate_ww_exclusions(
  ww_data_proc_set_corr_mat,
  outlier_col_name = "flag_as_ww_outlier",
  remove_outliers = TRUE
)
```
...
hosp_data_full <- rbind(
  ...
  hosp_data_proc_set_corr_mat %>%
    mutate(model_type = "Pre-set Corr. Matrix")
) %>%
  mutate(
    model_type = factor(
      model_type,
...
hosp_data_eval_full <- rbind(
  ...
  simulated_data_exp$hosp_data_eval %>%
    mutate(model_type = "Exponential"),
  simulated_data_set_corr_mat$hosp_data_eval %>%
    mutate(model_type = "Pre-set Corr. Matrix")
) %>%
  mutate(
    model_type = factor(
...
ww_data_eval_full <- rbind(
  ...
  simulated_data_exp$ww_data_eval %>%
    mutate(model_type = "Exponential"),
  simulated_data_set_corr_mat$ww_data_eval %>%
    mutate(model_type = "Pre-set Corr. Matrix")
) %>%
  mutate(
    model_type = factor(
...
rt_global_data_full <- rbind(
  ...
  simulated_data_set_corr_mat$rt_global_data %>%
    as.data.frame() %>%
    `colnames<-`(c("global_rt")) %>%
    mutate(
      model_type = "Pre-set Corr. Matrix",
      date = simulated_data_set_corr_mat$hosp_data_eval$date
    )
) %>%
  mutate(
    model_type = factor(
...
site_rt_data_full <- rbind(
  ...
  simulated_data_set_corr_mat$rt_site_data %>%
    t() %>%
    as.data.frame() %>%
    mutate(date = simulated_data_set_corr_mat$hosp_data_eval$date) %>%
    `colnames<-`(c(
      paste0("Site ", 1:n_sites),
      "Aux",
      "date"
...
site_rt_data_plot <- ggplot(
  site_rt_data_full %>%
    mutate(
      subpop = case_when(
        subpop == "Aux" ~ "Remainder of population",
        .default = subpop
      )
    )
) +
...
corr_matrix_df <- rbind(
  ...
  simulated_data_set_corr_mat$corr_matrix %>%
    as.data.frame() %>%
    `colnames<-`(c(
      paste0("Site ", 1:n_sites)
    )) %>%
...
## Fitting pre-set corr. matrix data
Finally, we fit the pre-set corr. matrix data generation model to all three of the
inference models.  Again, the inference models only differ by their assumed
site to site correlation structures.

<div class="fold s o">
```{r warning=FALSE, message=FALSE}
# Pre-set Corr. Matrix data to IID model ---------------------------------------
fit_set_to_iid <- wwinference::wwinference(
  ww_data = ww_data_to_fit_set_corr_mat,
  count_data = hosp_data_proc_set_corr_mat,
  forecast_date = forecast_date,
  calibration_time = calibration_time,
  forecast_horizon = forecast_horizon,
  model_spec = get_model_spec(
...
# ------------------------------------------------------------------------------
# Pre-set Corr. Matrix data to Exponential model -------------------------------
fit_set_to_exp <- wwinference::wwinference(
  ww_data = ww_data_to_fit_set_corr_mat,
  count_data = hosp_data_proc_set_corr_mat,
  forecast_date = forecast_date,
  calibration_time = calibration_time,
...
# ------------------------------------------------------------------------------
# Pre-set Corr. Matrix data to Unstructured model ------------------------------
fit_set_to_unstruct <- wwinference::wwinference(
  ww_data = ww_data_to_fit_set_corr_mat,
  count_data = hosp_data_proc_set_corr_mat,
  forecast_date = forecast_date,
  calibration_time = calibration_time,
...
# ------------------------------------------------------------------------------
# Pre-set Corr. Matrix data ----------------------------------------------------
set_pred_draws_df <- rbind(
  get_draws(fit_set_to_iid,
    what = "predicted_counts"
  )$predicted_counts %>%
    filter(draw %in% draws_to_keep) %>%
    mutate(
...
# ------------------------------------------------------------------------------
# Pre-set Corr. Matrix data ----------------------------------------------------
set_corr_matrix_draws_df <- rbind(
  fit_set_to_iid$fit$result$draws(
    variables = "non_norm_omega",
    format = "draws_array"
  ) %>%
    as.table() %>%
  ...
  fit_set_to_exp$fit$result$draws(
    variables = "non_norm_omega",
    format = "draws_array"
  ) %>%
    as.table() %>%
  ...
  fit_set_to_unstruct$fit$result$draws(
    variables = "non_norm_omega",
    format = "draws_array"
  ) %>%
    as.table() %>%
...
ww_result_plot <- ggplot(
  ...
  geom_point(
    data = ww_data_eval_full %>%
      mutate(gen_model_type = model_type),
    aes(x = date, y = log_genome_copies_per_ml_eval),
    shape = 21, color = "black", fill = "white"
  ) +
  geom_point(
    aes(x = date, y = observed_value)
...
site_rt_data_full <- site_rt_data_full %>%
  mutate(
...
rand_corr_matrix_result_plot <- ggplot(all_corr_matrix_draws_df %>%
  ...
  geom_vline(
    data = corr_matrix_df %>%
      filter(model_type == "Pre-set Corr. Matrix") %>%
      mutate(
        Row = Var1,
        Column = Var2
      ),
  ...
  ylab("") +
  xlab("Correlation Drawed") +
  ggtitle("Data Generation Model : Pre-set Corr. Matrix") +
  labs(
    color = "Assumed Corr. Structure",
    fill = "Assumed Corr. Structure"
  ) +
...
hosp_by_date_eval_forecast_plot <- ggplot( # nolint
  hosp_scores_forecast %>%
    ...
    filter(
      `Evaluation Score` %in% c("CRPS", "Bias")
    )
) +
  geom_point(
...
hosp_eval_forecast_plot <- ggplot(
  hosp_scores_forecast %>%
    ...
    filter(
      `Evaluation Score` %in% c("CRPS", "Bias", "ess/sec")
    )
) +
  geom_tile(
...
## Wastewater concentration evaluations
Here we prepare the evaluation metric plots for the wastewater concentrations.
We show these metrics for the nowcast and forecast for different viewing
windows.  We look at it by date, by site, by date/site, and averaged across
both date and site.  We also show quantile-quantile plots.


### Wastewater Concentrations at forecast & nowcast period evaluation scores
Again, here we prepare evaluation metric datasets for plotting.
```{r}
...
### Wastewater concentrations at forecast & nowcast period plots
Here we make the actual plots.
```{r}
# By Site
