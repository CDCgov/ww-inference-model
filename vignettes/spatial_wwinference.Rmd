...
## Central Rt Curve
Another requirement that we need to have based of the model definition, is a
central Rt curve that is the underlining behavior of the pathogen.  Here we
visualize the central Rt curve that is used in this vignette, but any central
Rt can be used here.
...
## Simulating data
Now we simulate the data with 3 different data generation models.  The
...
plant populations, total population, the global Rt standard deviation (we set
this to 0 to ensure central Rt is used without any deviations), and necessary
correlation function parameters.  To change the type of correlation structures
used is by setting `corr_function` to some correlation function made by the
user of one of 3 functions included in the package : `independence_corr_func_r`,
`exponential_decay_corr_func_r`, and `rand_corr_matrix_func`.  Another thing
that needs to be set is `corr_fun_params`, which are the parameters for the
correlation function passed into `corr_function`.  Under the hood, the
correlation matrix is created by calling `corr_function(corr_fun_params)`.  So,
`corr_function` and `corr_fun_params` need to be set to work in sync.  The
object that comes out of `generate_simulated_data` will have hospital admission
data, hospital admission data for eval, and global and site level Rt's.
...
## Preparing data
Here we take the output of the `generate_simulated_data` and prepare it for the
`wwinference` function that does the fitting to an inference model.
...
## Visualziing
With all of the data ready to fit, we first visualize what our simulations looks
like.  We visualize using the ggplot package.
...
Here we visualize both the global and site level Rt curves.  We have facets for
each of the data generation models.  We also look at a site called the Aux site,
which, per model definitions, is the population that is not captured by a
wastewater treatment plant.  We have a blue horizontal line at Rt = 1 for
reference of pathogen severity.
...
Similarly we can visualize the combined site level Rt curves.  Colors are the
different sites.  Again, we have a blue horizontal line at Rt = 1 for
reference of pathogen severity.
...
Here we do that, faceted by data generation model, we provide a scale that
gives level of correlation.  We also make the area of each circle correspond
with the correlation, so larger the area of the circle, larger the correlation.
To remind ourselves, IID will have no correlations between wastewater sites.
Exponential will have correlations between wastewater sites that are based off
the exponential correlation function that uses the distances between wastewater
treatment plants.  Rand. Corr. Matrix will have correlations between wastewater
treatment plants that are based off some unknown factors, to emulate this we
just set random correlations between sites.
...
## Fitting iid data
We start with fitting the IID data generation model to the 3 types of inference
models.  These 3 types of inference models are differing only by their assumed
correlation structure, as mentioned above.
...
We proceed to fit the exponential data generation model to the 3 types of
inference models.
...
Finally, we fit the rand. corr. matrix data generation model to all 3 of the
...
that uses the package's build in functionality.  This step is here to compare
the 3 data generation models agains the 3 inference models.
...
We start by looking at the raw draws of our fit object, then subset teh draws
by non_norm_omega, which is the matrix that contains nonnormalize correlations.
For more information on normalized vs nonnormalize correlations, refer to the
model definitions.  We then make a dataset that has a two columns indicating
which site to site pair the correlation value is for.  The last column is the
correlation value.  We also do this for each of the 9 fits we did above, and
combine them into one dataset iwth columns indicating waht data generation model
and inference model was used.
...
Here we showcase the hospital admissions and wastewater results.  Here we
facet again by the data generation model.  The colors represent the different
inference models, where the only difference between the inference models are
the assumed correlation structure.  Solid lines are medians of the
distributions, solid dots are data points used to in fitting, open dots are
for evaluation purposes.  The vertical dashed line represents the date that we
told the model to begin forecasting which ends based off the forecast horizon
specified in the model presets.  The nowcasting period will be from the period
from the last solid dot to the dashed line.  The forecast period is the period
after the dashed line.  Below hospital admissions, we have the wastewater
posterior predictive, with the same notations as the hospital admissions.

Exploring these results, we see that all models do relatively well to include
evaluation points, open dots, in their 95th quantiles.  Evaluation metrics will
be used to place numerical values on model performances, this section will be
...
Here we showcase the global and site level Rt's.  Notation is the same as
hospital admissions and wastewater results, except here the actual global and
site Rt curves are plotted for visual evaluation.  These actual curves are not
used by the model, so these curves must be inferred by the model.

We do very well to capture the actual Rt curves, there is some concern with how
high the inferred Rt reaches toward the nowcasting and forecasting periods.  This
is not surprising as there is no data there to guide the model for a more
accurate quantile.
...
# Numeric Results
For this section we explore how well our models did numerically.  The metrics
that we will be using are effective sample size per second (ess/sec), continuous
ranked probability score (CRPS), David-Sebastiani score (DSS), absolute error
(AE), squared error (SE), and bias.  All these, apart from ess/sec, will get
attained using the
