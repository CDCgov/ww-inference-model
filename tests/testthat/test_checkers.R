test_that("Test that check on maximum date errors as expected.", {
  date_vector <- seq(
...
test_that("Test that check on elements being non-negative works as expected.", {
  x <- c(1, 0, 7)
...
test_that("Test that check on non-missingness works as expected.", {
  x <- c(1, 0, 7)
...
test_that("Test that check on elements not repeating works as expected.", {
  x <- c(1, 0, 7)
...
test_that("Test that check on interger or characters works expected.", {
  x <- c(1, 0, 7)
...
test_that("Test that check on wastewater data column names works as expected.", {
  x <- tibble::tibble(
...
test_that("Test that check on count data column names works as expected.", {
  x <- tibble::tibble(
    date = lubridate::ymd("2024-01-01"),
    hosp = 5,
    pop = 100
...
test_that("Test that check on only a single value being present works as expected.", {
  x <- c(1)
...
test_that("Test that check on non-empty dataframe works as expected.", {
...
test_that("Test that check on non-empty tibble works as expected.", {
  ...
  empty_tibble_with_cols <- tibble::tibble(
    Col1 = numeric(),
    Column2 = character()
  )
  expect_error(assert_df_not_empty(
    empty_tibble_with_cols,
    "empty_tibble_with_cols"
  ))
})


test_that("Test that check on daily dates works as expected.", {
  daily_dates <- c(
...
test_that("Test that validate pmfs returns the expected error message.", {
  invalid_pmf <- c(0.4, 0.4, 0.4)
  expect_error(validate_pmf(invalid_pmf), regexp = "does not sum to 1")
})

test_that("Test that assert dates in range function works as expected.", {
