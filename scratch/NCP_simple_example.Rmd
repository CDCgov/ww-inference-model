...
"
comp_stan_model1_centered <- cmdstanr::write_stan_file(
  stan_model1_centered
)
model1_cent <- cmdstanr::cmdstan_model(
  comp_stan_model1_centered
)
```
...
"
comp_stan_model1_non_centered <- cmdstanr::write_stan_file(
  stan_model1_noncentered
)
model1_non_cent <- cmdstanr::cmdstan_model(
  comp_stan_model1_non_centered
)
```
# Fit the model
```{r}
fit_non_cent <- model1_non_cent$sample(
  data_list
)
```
...
"
comp_stan_model2_centered <- cmdstanr::write_stan_file(
  stan_model2_centered
)
model2_cent <- cmdstanr::cmdstan_model(
  comp_stan_model2_centered
)
```
Fit the centered model
```{r}
fit_cent <- model2_cent$sample(
  data_list
)
```
...
"
comp_stan_model2_non_centered <- cmdstanr::write_stan_file(
  stan_model2_noncentered
)
model2_non_cent <- cmdstanr::cmdstan_model(
  comp_stan_model2_non_centered
)
```
Fit the non-centered parameterization with a negative binomial observation process
```{r}
fit_cent <- model2_non_cent$sample(
  data_list
)
```
