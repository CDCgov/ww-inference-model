...
exponential_corr <- function(d_ij, phi, l) {
  return(exp(-(d_ij / phi)^l))
}
phi <- 0.2
l <- 1
v <- 2
d <- seq(0, 1000, length.out = 1000)

# main
exponential_values <- sapply(d / max(d), exponential_corr, phi = phi, l = l)
ggplot() +
  geom_line(
    aes(x = d / max(d), y = exponential_values),
    size = 1.25,
    color = "blue"
  ) +
  labs(title = "Exponential Correlation Function") +
