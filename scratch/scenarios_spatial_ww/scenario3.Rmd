...
sigma_sqrd_generalized <- 0.025^n_sites
custom_corr_func_param <- list(
  corr_matrix = matrix(
    data = c(
      1, .25, .25, .95,
      .25, 1, .05, .25,
...
simulated_data_rng_corr_mat <- withr::with_seed(1, {
  wwinference::generate_simulated_data(
    ...
    global_rt_sd = global_rt_sd,
    use_spatial_corr = TRUE,
    corr_function = wwinference::custom_corr_matrix_func,
    corr_fun_params = custom_corr_func_param,
    sigma_sqrd_generalized = sigma_sqrd_generalized,
    aux_site_bool = TRUE
  )
})
