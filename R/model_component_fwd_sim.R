...
subpop_rt_process <- function(n_subpops, r_weeks, subpop_level_rt_variation) {
...
get_time_varying_daily_ihr <- function(
  p_hosp_mean,
  uot,
  ot,
  ht,
  ...
  p_hosp_w_logit <- p_hosp_int_logit +
    rnorm(
      tot_weeks - 1,
      0,
      p_hosp_w_sd_sd
...
get_pred_subpop_gen_per_n <- function(
  convolve_fxn,
  n_sites,
  uot,
  ot,
...
downsample_for_frequency <- function(
  log_conc_lab_site,
  n_lab_sites,
  ot,
  ht,
...
format_ww_data <- function(
  ...
  ww_data <- as.data.frame(t(log_obs_conc_lab_site)) |>
    ...
    dplyr::left_join(site_lab_map, by = "lab_site") |>
    dplyr::left_join(
      data.frame(
...
#' population size
format_hosp_data <- function(pred_obs_hosp, dur_obs, pop_size, date_df) {
...
format_subpop_hosp_data <- function(
...
#' @export
calc_rt <- function(new_i, convolve_fxn, generation_interval, uot, tot_time) {
  rt <- (new_i /
    convolve_fxn(
      new_i,
      rev(c(0, generation_interval)),
      tot_time # Assert no contributions
...
create_site_lab_map <- function(site, lab, ww_pop_sites) {
