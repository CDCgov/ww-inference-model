...
get_draws.data.frame <- function(x,
  ...
  count_draws <- if (what_ok["predicted_counts"]) {
    draws |> # predicted_counts
      ...
      dplyr::select(
        "date", "draw", "observed_value",
        "pred_value", "total_pop"
      )
  } else {
    NULL
  }
  ...
  ww_draws <- if (what_ok["predicted_ww"]) {
    draws |>
      ...
      dplyr::select(
        "below_lod", "date", "draw", "exclude",
        "flag_as_ww_outlier",
        "lab", "lab_site_index", "lab_site_name",
        "log_genome_copies_per_ml",
        "log_lod", "observed_value", "pred_value",
        "site", "site_index",
        "site_pop", "subpop"
      )
  } else {
  ...
  subpop_rt_draws <- if (what_ok["subpop_rt"]) {
    draws |>
      ...
      dplyr::select(
        "date", "draw", "pred_value", "site", "site_index",
        "site_pop", "subpop"
      )
  } else {
    NULL
